...
| Task 3 | 实现启动加载、完整校验和 JVM 原子索引替换 | pending |
| Task 4 | 实现精确 Map 和任意位置连续子串 List 匹配 | pending |
| Task 5 | 实现启动刷新与每日定时刷新 | pending |
| Task 6 | 在 `IntentRoutingNode` 接入 `StockRequestResolver` 和双维度澄清 | pending |
| Task 7 | 实现 Redis Pending、生命周期和 SSE 澄清文本 | pending |
| Task 8 | 补齐单元测试、集成测试、Pending 和回归测试 | pending |
| Task 9 | 验证内存、查询性能、7 天过期和真实 Tushare 数据 | pending |
