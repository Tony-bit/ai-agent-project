...
现有界面将这个未知状态渲染为红色“操作失败”。同时，`auto_agent` 经意图路由进入交易
...
SSE 事件
  -> normalizeAgentEvent
  -> classifyAgentEvent
      -> 成功终态：terminalSeen=true, outcome=completed
      -> 错误终态：terminalSeen=true, outcome=failed
      -> 非终态结果：仅渲染内容

流结束（EOF）
  -> terminalSeen=true：保留已判定的终态
  -> terminalSeen=false 且无协议错误：outcome=indeterminate
      -> hasResult=true：显示警告并保留结果
      -> hasResult=false：显示警告和重试提示
...
独立交易接口继续由 `TradingSseSession` 的单一写入器（`writer`）负责写出业务事件和关闭连接。

`auto_agent` 内嵌交易路径遵循以下所有权：

```text
AiAgentController / AutoAgentExecuteStrategy 管理 emitter 生命周期
  -> IntentRoutingNode 启动 TradingStarter
  -> TradingStarter 发送 final_decision
  -> TradingStarter 发送 trading_complete
  -> TradingStarter 返回，但不关闭外层 emitter
  -> AutoAgentExecuteStrategy 执行剩余清理工作
  -> AutoAgentExecuteStrategy 仅关闭 emitter 一次
```

交易子流程只负责发送交易业务终态，不关闭外层 `auto_agent` 的 `emitter`。这样可以避免子流程关闭
传输通道后，外层仍在持久化和清理的生命周期倒置。

### 4.4 终态发送结果

`TradingStateContext.sendTerminalCompleteOnce()` 和 `sendTerminalErrorOnce()` 必须返回真实发送
结果。终态方法应调用能够返回布尔值的发送实现，不能在 `sender` 为空、`sink` 拒绝、连接已断开
或 `emitter.send()` 抛异常后仍返回 `true`。

对于原始 `emitter` 兼容路径，事件发送和最终关闭应由同一个所有者按顺序执行。对于 `sink` 路径，
...
```text
INFO  SSE 终态已接受: sessionId=..., type=trading_complete, path=emitter
INFO  SSE 终态已接受: sessionId=..., type=trading_complete, path=sink
WARN  SSE 终态被拒绝: sessionId=..., type=trading_complete, state=...
WARN  SSE 终态发送失败: sessionId=..., type=trading_complete, error=...
INFO  SSE emitter 已关闭: sessionId=..., owner=auto_agent, terminalSent=true
```
...
| 用例 | 输入 | 预期 |
| --- | --- | --- |
| 通用完成 | 结果事件 + `complete` + EOF | `completed`，显示任务完成 |
| 交易完成 | `final_decision` + `trading_complete` + EOF | `completed`，显示任务完成 |
| 明确业务失败 | 结果事件 + `error` + EOF | `failed`，显示操作失败 |
| 读取异常 | 部分结果 + `reader` 拒绝读取 | `failed`，显示网络或读取错误 |
| 协议异常 | 格式错误的事件 + EOF | `failed`，显示协议错误 |
| 有结果无终态 | 最终结果 + EOF | `indeterminate`，显示中性警告并保留结果 |
| 无结果无终态 | 进度事件 + EOF | `indeterminate`，显示未确认状态和重试提示 |
| 用户取消 | `AbortError` | `cancelled`，不显示操作失败 |

### 6.2 后端单元测试

- `sendTerminalCompleteOnce()` 在 `sender` 发送成功时返回 `true`。
- `sender` 抛出异常、`sink` 拒绝或 `emitter` 已关闭时返回 `false`。
- 成功终态最多发送一次。
...
- 模拟终态 `sender` 发送失败，验证没有成功日志，前端进入 `indeterminate` 或明确读取失败。
- 模拟后端错误事件，验证前端进入 `failed`，且状态不被已有结果覆盖。
...
- `AutoAgentExecuteStrategy`：作为 `auto_agent` 的 `emitter` 唯一关闭所有者，在清理结束后收口连接。
...
- 移除交易子流程对外层 `emitter` 的关闭后，必须保证外层策略的所有返回路径都能关闭 `emitter`。
...
5. 终态发送接口必须返回真实结果，关闭日志不能替代终态送达日志。
6. 独立交易接口继续复用现有 `TradingSseSession` 单一写入器（`writer`）和队列排空机制。
