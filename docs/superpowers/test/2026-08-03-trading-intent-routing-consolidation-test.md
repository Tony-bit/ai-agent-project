...
|------|------|------|------|------|------|
| TC-001 | 完整名称启动分析 | 3201 可调用名称搜索 | 对药明康德进行完整投资分析 | 唯一解析为 `603259.SH`，创建一个新 run 并执行 pipeline | pass |
| TC-002 | 六位代码启动分析 | Provider 返回唯一身份 | `603259` | 不调用名称搜索，权威校验后启动 Trading | pass |
| TC-003 | 标准代码启动分析 | Provider 返回唯一身份 | `603259.SH` | ticker 保持为权威 `603259.SH` | pass |
| TC-004 | 分析类型单选 | 类型 Mapper 可用 | `TECHNICAL` | 只选择 `TECHNICAL` 分析师 | pass |
| TC-005 | 分析类型组合 | 类型 Mapper 可用 | `FUNDAMENTAL,NEWS` | 选择对应两个分析师 | pass |
| TC-006 | 默认全部分析师 | 类型为空或 `ALL` | 空值、`ALL` | 使用 Trading 当前默认全部分析师 | pass |
| TC-007 | 已验证目标启动 | 身份预检成功 | 已验证 `TargetContext` | `TradingStarter` 使用同一对象且不重复查询身份 | pass |
| TC-008 | 连续换股分析 | 同一 session | 药明康德后分析兆易创新 | 两次 runId 不同，第二次标的和 Trading 上下文不含第一只股票 | pass |
| TC-009 | 连续同股分析 | 同一 session | 连续两次相同 Query | 两次 pipeline 均执行，runId 不同 | pass |
| TC-010 | 通用追问复用历史 | 前一轮 Trading 已持久化 | 刚才为什么建议持有 | 3201/GENERAL_CHAT 能读取主会话历史，不启动旧 Trading run | pass |
| TC-011 | 普通多任务执行 | 任务列表不含股票分析 | PE 与通用问答组合 | 沿用现有多任务执行和汇总 | pass |

### 3.2 异常场景

| 编号 | 场景名称 | 前置条件 | 输入 | 预期结果 | status |
|------|------|------|------|------|------|
| TC-101 | 股票身份不存在 | Provider 返回空列表 | 合法六位代码 | 登记 `CLARIFICATION`，不创建 runId，不调用 `TradingStarter` | pass |
| TC-102 | 身份 Provider 超时 | Provider 抛超时异常 | 合法代码 | 登记 `ERROR`，保留 cause，不调用 `TradingStarter` | pass |
| TC-103 | 身份 Provider 鉴权失败 | Provider 抛鉴权异常 | 合法代码 | 返回数据服务错误，不伪装成澄清 | pass |
| TC-104 | 权威身份返回多条 | Provider 返回两条 | 合法代码 | 抛 `StockIdentityValidationException` 并停止 | pass |
| TC-105 | 权威记录非法 | targetId 非法或名称为空 | 合法代码 | 返回身份校验错误并记录告警 | pass |
| TC-106 | 请求名称与权威名称不一致 | Provider 返回另一名称 | ticker 与名称冲突 | 不进入 Trading pipeline | pass |
| TC-107 | 股票槽位为空 | 路由为股票分析 | 无 `StockSlot` | 发送一次澄清和一次完成事件 | pass |
| TC-108 | ticker 非法 | 路由为股票分析 | 五位、七位、字母代码 | 发送路由澄清，不查询 Provider | pass |
| TC-109 | 非 A 股后缀 | 路由为股票分析 | `.HK`、`.US` | 拒绝并停止，不创建 run | pass |
| TC-110 | 股票多任务门禁 | `multiTask=true` | 股票分析加通用任务 | 整轮拒绝，所有子任务均未执行 | pass |
| TC-111 | Provider 错误 SSE 协议 | 已登记 `ERROR` | Provider 失败 | 只发送一个已完成 `error`，不追加澄清或完成事件 | pass |
| TC-112 | 澄清 SSE 协议 | 已登记 `CLARIFICATION` | 股票不存在 | 依次发送一次 `summary/clarification` 和一次 `complete` | pass |
| TC-113 | SSE 发送失败 | Sink 拒绝或客户端断开 | 任一路由终止结果 | 不重发，控制流正常返回并由外层清理 | pass |
| TC-114 | 未知 Trading Tool | 白名单包含不存在名称 | 应用装配 | 启动失败并指出未知工具名 | pass |
| TC-115 | 3201 非 JSON 输出 | 统一路由解析失败 | Markdown 报告 | 沿用重试/降级，不进入 Trading | pass |

### 3.3 边界场景

| 编号 | 场景名称 | 前置条件 | 输入 | 预期结果 | status |
|------|------|------|------|------|------|
| TC-201 | ticker 空格与小写后缀 | Provider 返回唯一身份 | ` 603259.sh ` | 规范化为 `603259.SH` | pass |
| TC-202 | 直接代码无名称 | Provider 返回唯一身份 | `stockName=null` | 使用权威名称创建 `TargetContext` | pass |
| TC-203 | 废弃 exchange 冲突 | 旧请求可反序列化 | `603259` 加 `exchange=SZ` | 忽略 exchange，以权威 `603259.SH` 为准 | pass |
| TC-204 | 代码自身后缀错误 | Provider 无对应记录 | `603259.SZ` | 按身份未找到停止，不使用 exchange 修正 | pass |
| TC-205 | 类型大小写和空格 | Mapper 可用 | ` technical ` | 映射为 `TECHNICAL` | pass |
| TC-206 | 类型组合含非法项 | Mapper 可用 | `TECHNICAL,UNKNOWN` | 忽略非法项，保留 `TECHNICAL` | pass |
| TC-207 | 类型全部非法 | Mapper 可用 | `UNKNOWN` | 降级为默认全部并记录警告 | pass |
| TC-208 | Tool 白名单缺少 Client | Client 不在 Map | 任意 Client | 不装配任何 Trading Tool | pass |
| TC-209 | Tool 白名单为空 | Client 显式空列表 | 任意 Client | 不装配任何 Trading Tool | pass |
| TC-210 | Tool 白名单重复项 | 配置包含重复工具名 | 3201 配置 | 去重后工具集合稳定 | pass |
...
| TC-306 | PE 与巡检回归 | `UNIFIED` 模式 | PE、巡检请求 | 原节点正常执行 | pass |
| TC-307 | analysisDepth 回归 | 前一轮询问分析深度 | 完整投资分析 | 恢复股票对象并进入新 Trading run | pass |
...
| TC-315 | 新旧库最终状态一致 | 旧库升级与空库全迁移数据集 | 执行迁移模型 | 两类路径最终配置一致，3201 与 6002-6013 不变 | pass |
...
|------|------|------|
| AC-001~AC-010 | TC-001~TC-009、TC-115、TC-301~TC-302、TC-309 | pass |
| AC-011~AC-017 | TC-114、TC-208~TC-210、TC-303~TC-308 | pass |
| AC-018~AC-022 | TC-010~TC-011、TC-107、TC-110~TC-113 | pass |
| AC-023~AC-025 | TC-004~TC-006、TC-205~TC-207 | pass |
| AC-026~AC-029 | TC-007、TC-101~TC-106、TC-202、TC-308 | pass |
| AC-030~AC-033 | TC-301、TC-313~TC-315 | pass |
| AC-034 | 全部股票用例固定 `UNIFIED`；`SPLIT` 明确排除 | pass |
| AC-035~AC-037 | TC-003、TC-201、TC-203~TC-204、TC-312 | pass |
| AC-038~AC-041 | TC-008~TC-010、TC-310~TC-311 | pass |
...
| 步骤 | 内容 | 预期结果 | status |
|------|------|------|------|
| 1 | 补充 domain 路由、工具白名单和 Root 单元测试 | 正常、异常和边界分支断言完成 | pass |
| 2 | 补充 trading-domain 类型映射、身份预检和 Starter 测试 | 请求构造、异常分类和 run 隔离通过 | pass |
| 3 | 补充 app 迁移静态测试与跨模块集成测试 | 6001 精确删除及主链路通过 | pass |
| 4 | 补充 trigger 直接 Trading API 回归 | 原接口契约通过 | pass |
| 5 | 执行 `mvn -pl ai-agent-study-domain -am test` | domain 测试通过 | pass |
| 6 | 执行 `mvn -pl ai-agent-study-trading/ai-agent-study-trading-domain -am test` | trading-domain 测试通过 | pass |
| 7 | 执行 `mvn -pl ai-agent-study-app,ai-agent-study-trigger -am test` | 集成与接口回归通过 | pass |
| 8 | 执行 `mvn test` | 全仓测试通过 | pass |
...
| 编号 | 验收项 | 标准 | status |
|------|------|------|------|
| AC-T01 | 正常主流程 | TC-001~TC-011 全部通过 | pass |
| AC-T02 | 异常处理 | TC-101~TC-115 全部通过 | pass |
| AC-T03 | 边界行为 | TC-201~TC-210 全部通过 | pass |
| AC-T04 | 核心回归 | TC-301~TC-315 全部通过 | pass |
| AC-T05 | Story 覆盖 | Story AC-001~AC-041 均有测试映射且通过 | pass |
| AC-T06 | 数据库兼容 | 新旧库到 `V2030` 状态一致且 Prompt 6001 保留 | pass |
| AC-T07 | 编译与全仓测试 | `mvn test` 成功，无既有测试回归 | pass |
| AC-T08 | SSE 生命周期 | 终止协议、发送次数和 emitter 所有权符合设计 | pass |
...
| 项目 | 结果 | status |
|------|------|------|
| 单元测试 | 全仓 961 个测试通过 | pass |
| 集成测试 | 跨模块定向 4 个测试通过 | pass |
| 接口回归 | 直接 Trading API 4 个测试通过 | pass |
| MySQL 手工验收 | 未执行，需真实 MySQL 环境 | append |
| 编译与全仓测试 | `mvn clean compile -q` 与 `mvn test` 成功 | pass |

### 10.2 问题记录

| 编号 | 问题描述 | 影响范围 | status |
|------|------|------|------|
| BUG-001 | Few-shot 旧股票样本缺少统一槽位 | app 全量回归；已迁移 10 条样本 | pass |

### 10.3 结论

- 是否达到提测/合并条件：是（自动化门禁）。
- 当前结论：Task 1~Task 7、全部 TC/AC、编译与全仓自动化测试通过；真实 MySQL 与前端手工验收仍需在发布环境执行。
