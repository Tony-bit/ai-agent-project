...
|---|---|---|---|---|---|---|---|
| TC-001 | P0 | 通用 Agent 流式对话 | 页面空闲，通用接口可用 | 输入合法问题并发送，等待完整流 | 内容增量更新；最终内容仅出现一次并进入结果面板；收到 `complete` 后 Loading 与按钮恢复 | 可控 + 真实 | pass |
| TC-002 | P0 | 股票多 Agent 分析 | 股票分析及行情依赖可用 | 输入合法股票代码、日期和轮次并发送 | 分析师进度可读；最终决策进入结果面板；`trading_complete` 只收口一次 | 可控 + 真实 | pass（存在 BUG-002 展示问题） |
...
| TC-210 | P2 | 连续 Toast | 短时间触发多个校验或错误 | 连续触发 3 次提示 | 页面最多保留一个活动 Toast；最新内容正确；错误使用 alert 语义，其余使用 status | 可控 + 手工 | pass |

### 5.4 回归场景

| 编号 | 优先级 | 场景名称 | 前置条件 | 操作 | 预期结果 | 执行链路 | status |
|---|---|---|---|---|---|---|---|
| TC-301 | P0 | 统一入口意图路由 | 真实后端可用 | 自由输入不同意图并等待完成 | 原有路由及下游执行可用；阶段摘要安全展示；最终结果正确归类 | 真实 | pass |
| TC-302 | P1 | 多任务拆解与汇总 | 真实后端可用 | 输入包含多个独立或依赖任务的请求 | 子任务过程、依赖执行和最终汇总仍可完成；无前一请求缓存污染 | 真实 | blocked（本轮未执行多任务输入） |
| TC-303 | P0 | 股票多 Agent 协作 | 行情和模型依赖可用 | 执行完整股票分析 | 分析师、辩论、风控和最终决策链路仍可观察并正常终止 | 真实 | pass（`600519` 最终决策 HOLD） |
| TC-304 | P1 | 跨会话 Persona/情景记忆 | 同一用户可同步记忆 | 会话 A 同步记忆，新建会话 B 后进行相关追问 | 原有跨会话记忆能力不变；不同 `userId` 之间不串数据 | 真实 | blocked（Mem0 上游余额不足） |
| TC-305 | P1 | 会话列表与消息分页 | 历史数据超过一页 | 滚动加载列表和更早消息 | 游标正确；无重复或丢失；切换会话后内容与会话 ID 一致 | 真实 | blocked（列表与历史恢复通过，缺少超过一页的测试数据） |
| TC-306 | P1 | 新建、切换与清空会话 | 存在历史和活动请求 | 依次新建、切换、清空并返回业务页面 | 会话状态、空态和按钮正确；不遗留上一个请求的视觉状态 | 真实 + 手工 | blocked（新建与切换通过；未执行破坏性的清空动作） |
| TC-307 | P2 | 输入校验和非阻塞反馈 | 页面空闲 | 提交空问题、非法股票代码/日期并快速修正 | 不发送无效请求；不使用阻塞式 alert；Toast 后输入与页面仍可操作 | 可控 + 手工 | fail（BUG-001：页面示例为美股代码，后端仅接受 6 位 A 股代码） |
...
| 步骤 | 操作 | 预期结果 | status |
|---|---|---|---|
| 1 | 在 `1280×720` 完成通用、交易、取消和记忆同步 | 核心流程稳定，控件无遮挡 | blocked（通用、交易、取消通过；记忆同步受外部余额限制） |
| 2 | 在 `390×844` 重复核心操作并使用键盘导航 | 布局可用，焦点和状态语义清晰 | pass |
| 3 | 长流期间回看历史并恢复到底部 | 滚动控制符合用户意图 | pass |
| 4 | 触发失败、取消、空态和未知事件 | 文案、Toast、状态点和恢复行为一致 | pass |
...
| 编号 | 验收项 | 标准 | status |
|---|---|---|---|
| AC-001 | 流式正确性 | 半包、多事件、CR/LF、多 data 行和结束冲刷全部通过 | pass |
| AC-002 | 安全渲染 | 实时、历史、用户输入和元数据路径中的恶意内容均不执行 | pass |
| AC-003 | 完成语义 | 消息完成与请求终止分离，每条流只收口一次 | pass |
| AC-004 | 生命周期 | 成功、失败、取消、断流后均可继续下一请求 | pass |
| AC-005 | 竞态隔离 | 取消 A 后立即启动 B，A 的迟到回调不影响 B | pass |
| AC-006 | 运行配置 | 同源、URL/localStorage 覆盖、校验和默认回退符合设计 | pass |
| AC-007 | 响应式与可访问性 | `1280×720`、`390×844` 核心操作可用，状态可被辅助技术识别 | pass |
| AC-008 | 长流体验 | 不逐 chunk 全量解析 Markdown，不抢用户滚动位置 | pass |
| AC-009 | 核心回归 | 通用 Agent、股票分析、会话、记忆和多任务无阻塞回归 | blocked（通用、股票、会话通过；记忆和未执行场景仍阻塞） |
| AC-010 | 自动化回归 | Node 与 Maven 默认测试无本轮新增失败 | pass |
...
| 交易协议事件映射不完整 | 大量正常进度显示为“未知事件/未知阶段”，并直接展示原始 JSON | 对齐真实交易 SSE type/subType，增加稳定标签与结构化摘要 |
...
|---|---|---|---|---|---|
| ENV-001 | TC-001～TC-004、TC-008、TC-301～TC-306 | 单模块启动引用旧依赖，完整 Reactor 安装后 Bean 装配正常 | 阻塞 | 真实 Agent、交易、会话与记忆 E2E | resolved |
| ENV-002 | TC-004、TC-304 | Mem0 调用上游模型返回 402 `Insufficient Balance` | 阻塞 | 记忆同步、跨会话记忆 | open |
| BUG-001 | TC-307 | 股票输入 placeholder 为 `AAPL、NVDA`，真实后端仅接受 6 位 A 股代码 | P2 | 金融分析输入引导 | open |
| BUG-002 | TC-002、TC-303 | 真实交易流大量合法进度被标为“未知事件/未知阶段”，部分原始 JSON 直接展示 | P1 | 金融分析过程可读性与产品体验 | open |

### 12.4 结论

- 是否达到提测条件：`是（前端自动化、浏览器冒烟与响应式检查通过）`
- 是否达到发布/演示条件：`否（BUG-002 未修复，记忆同步仍受外部余额限制）`
- 结论规则：全部验收项有结果、P0/P1 用例通过且不存在未关闭阻塞缺陷后，方可改为“是”。

---

## 13. 2026-07-20 登录与用户级会话隔离回归

### 13.1 自动化结果

| 验证项 | 命令/环境 | 结果 |
|---|---|---|
| 前端核心单测 | `node --test docs/dev-ops/nginx/html/test/agent-ui-core.test.js` | pass，32/32 |
| 后端聚焦回归 | Maven reactor，执行认证、JWT 过滤器、所有权、操作租约、事务删除、会话 API、Agent/Trading 守卫测试 | pass，退出码 0 |
| 编译回归 | `mvn clean compile -q` | pass，11 个 reactor 模块编译成功 |
| 浏览器安全与交互冒烟 | Edge Headless + Playwright，`agent-ui-security-smoke.html` | pass，18/18 |
| 桌面布局 | Edge Headless，`1280x720` | pass，无横向溢出，登录与聊天关键控件均可见 |
| 移动布局 | Edge Headless，`390x844` | pass，文档宽度 390px，高度 844px；状态栏、设置、同步和发送按钮均位于页面边界内 |

### 13.2 覆盖内容

- 账号登录、游客创建、token 保存、`/auth/me` 恢复、退出和 401 清理。
- 会话列表、历史、Agent、股票分析、记忆同步和删除请求使用 Bearer token，不再传递可信 `userId`。
- `AVAILABLE`、`OWNED`、`UNAVAILABLE` 所有权判定，以及执行/删除原子互斥。
- 消息与会话的事务删除顺序、零影响行失败、提交后缓存清理和 Mem0 保留语义。
- 会话 ID 设置校验、焦点恢复、Escape、删除阻止冒泡、取消确认和当前/非当前删除状态。
- 桌面和移动登录页、认证后工作区的溢出与关键控件边界检查。

### 13.3 剩余环境验证

| 项目 | 状态 | 说明 |
|---|---|---|
| 真实 MySQL 初始化与账号登录 | blocked | 本轮未修改或连接共享数据库；需在目标环境执行认证用户 DDL/DML 后补跑 |
| 真实 Mem0 保留验证 | blocked | 自动化已证明删除服务无 Mem0 删除依赖；仍需在具备可用 Mem0 账号的环境确认长期记忆可继续查询 |
| 多实例执行/删除互斥 | out-of-scope | 当前 `SessionOperationRegistry` 是单 JVM 实现；多实例部署前需迁移到 Redis 或数据库租约 |
