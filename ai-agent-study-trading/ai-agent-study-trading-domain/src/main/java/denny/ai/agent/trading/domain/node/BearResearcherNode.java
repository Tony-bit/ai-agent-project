...
public class BearResearcherNode extends AbstractExecuteSupport {
    ...
    private NarrativeNodeResult prepareInternal(TradingContextVO context,
        ...
        NarrativeNodeResult bearThesis = generateBearThesis(context, dynamicContext);
    ...
    private NarrativeNodeResult generateBearThesis(TradingContextVO context,
                                      DefaultAutoAgentExecuteStrategyFactory.DynamicContext dynamicContext) {
        ...
        log.debug("LLM streaming input | operation=BearResearcherNode | content=\n{}", prompt);
        String response = denny.ai.agent.trading.domain.execution.TradingLlmCallAudit.execute(
                context, "6007", "BearResearcherNode",
        ...
        if (denny.ai.agent.trading.domain.prompt.TradingPromptModeResolver.requireMode(dynamicContext)
            ...
            return new NarrativeNodeResult("BEAR", response);
        }
