...
class Memory(MemoryBase):
    ...
    def add(
        ...
        *,
        ...
        # Trigger persona update after successful semantic memory storage (non-blocking)
        if effective_filters.get("user_id"):
            try:
                self._update_persona(effective_filters["user_id"])
            except Exception as e:
                logger.warning(f"Persona update failed for user {effective_filters['user_id']}: {e}")
    ...
    def delete(self, memory_id):
        ...
        # Cascade delete persona for this user
        user_id = existing_memory.payload.get("user_id")
        if user_id:
            try:
                self._delete_persona_if_exists(user_id)
            except Exception as e:
                logger.warning(f"Failed to delete persona for user {user_id}: {e}")
    ...
    def _create_memory(self, data: str, existing_embeddings: Union[Dict[str, List[float]], List[float]], metadata=None):
        ...
        # Use provided hash if available (e.g., for persona), otherwise compute from data
        if "hash" not in new_metadata:
        ...
        new_metadata.setdefault("type", MemoryType.SEMANTIC.value)
        ...
        self.db.add_history(
            ...
            memory_type=new_metadata.get("type"),
    ...
    def _update_memory(self, memory_id, data: str, existing_embeddings: Union[Dict[str, List[float]], List[float]], metadata=None):
        ...
        if "type" not in new_metadata and "type" in existing_memory.payload:
            new_metadata["type"] = existing_memory.payload["type"]
    ...
    # =========================================================================
    # Persona (User Profile) Methods
    # =========================================================================

    def get_persona(self, user_id: str) -> Optional[str]:
        """
        Get user persona as a natural language text.

        If persona exists, returns it directly (O(1) lookup).
        If not, generates from semantic memories, stores it, and returns.

        Args:
            user_id: The user identifier.

        Returns:
            Natural language persona text, or None if generation fails.
        """
        memory_id = self._get_persona_memory_id(user_id)
        if memory_id:
            memory = self.vector_store.get(vector_id=memory_id)
            if memory and memory.payload.get("data"):
                return memory.payload["data"]

        return self._generate_persona_from_semantic_memories(user_id)

    def get_persona_metadata(self, user_id: str) -> Optional[Dict[str, Any]]:
        """
        Get user persona metadata dict (programmatic access).

        Args:
            user_id: The user identifier.

        Returns:
            The persona metadata dict, or None if not found.
        """
        memory_id = self._get_persona_memory_id(user_id)
        if not memory_id:
            return None
        memory = self.vector_store.get(vector_id=memory_id)
        if memory:
            return memory.payload.get("metadata")
        return None

    def _get_persona_memory_id(self, user_id: str) -> Optional[str]:
        """Query the history table for the persona record memory_id."""
        return self.db.get_memory_id(user_id, memory_type="persona")

    def _extract_persona_attributes(
        self, facts: List[str], max_retries: int = 3
    ) -> Optional[Dict[str, Any]]:
        """
        Extract persona attributes from a list of facts using LLM.

        Args:
            facts: List of fact strings from semantic memories.
            max_retries: Number of retry attempts on failure.

        Returns:
            Persona attributes dict, or None on failure.
        """
        from mem0.configs.prompts import PERSONA_EXTRACTION_PROMPT

        facts_text = "\n".join(f"- {fact}" for fact in facts)
        prompt = PERSONA_EXTRACTION_PROMPT.format(facts=facts_text)

        required_fields = ["user_name", "occupation", "personality",
                          "preferences", "communication_style", "skills"]
        ...
        for attempt in range(max_retries):
            try:
                ...
                try:
                    attrs = json.loads(cleaned, strict=False)
                except json.JSONDecodeError:
                    import re
                    json_match = re.search(r'\{[\s\S]*\}', cleaned)
                    if json_match:
                        attrs = json.loads(json_match.group(), strict=False)
                    else:
                        logger.warning(f"Failed to parse JSON from response: {cleaned[:200]}")
                        continue

                if all(field in attrs for field in required_fields):
                    return attrs
                else:
                    missing = [f for f in required_fields if f not in attrs]
                    logger.warning(f"Persona attributes missing fields {missing}: {attrs}")
            except Exception as e:
                logger.warning(f"Persona extraction failed (attempt {attempt + 1}/{max_retries}): {e}")

        logger.warning("Persona extraction failed after all retries")
        return None

    def _generate_persona_text(self, attributes: Dict[str, Any]) -> str:
        """
        Generate natural language persona text from attributes dict.

        Args:
            attributes: Persona attributes dict.

        Returns:
            Natural language persona text.
        """
        from mem0.configs.prompts import PERSONA_TEXT_GENERATION_PROMPT

        prompt = PERSONA_TEXT_GENERATION_PROMPT.format(
            extracted_attributes_json=json.dumps(attributes, ensure_ascii=False, indent=2)
        )
        response = self.llm.generate_response(messages=[{"role": "user", "content": prompt}])
        return remove_code_blocks(response).strip()

    def _generate_persona_from_semantic_memories(self, user_id: str) -> Optional[str]:
        """
        Generate persona from semantic memories when no persona record exists.

        Args:
            user_id: The user identifier.

        Returns:
            Generated persona text, or None on failure.
        """
        max_retries = 3
        for attempt in range(max_retries):
            try:
                # Search semantic memories for this user
                search_results = self.vector_store.search(
                    query="职业 性格 偏好 沟通风格 技能",
                    vectors=self.embedding_model.embed("职业 性格 偏好 沟通风格 技能", "search"),
                    top_k=50,
                    filters={"user_id": user_id},
                )
                if not search_results:
                    logger.warning(f"No semantic memories found for user {user_id}, cannot generate persona")
                    return None

                # Extract facts from search results
                facts = [mem.payload.get("data", "") for mem in search_results
                         if mem.payload.get("data")]

                if not facts:
                    logger.warning(f"No fact data in semantic memories for user {user_id}")
                    return None

                # Extract persona attributes
                attrs = self._extract_persona_attributes(facts)
                if not attrs:
                    logger.warning(f"Failed to extract persona attributes for user {user_id}")
                    continue

                # Generate natural language text
                persona_text = self._generate_persona_text(attrs)

                # Compute hash and create memory record
                persona_hash = hashlib.md5(
                    json.dumps(attrs, sort_keys=True, ensure_ascii=False).encode()
                ).hexdigest()

                embeddings = self.embedding_model.embed(persona_text, "add")
                self._create_memory(
                    data=persona_text,
                    existing_embeddings={persona_text: embeddings},
                    metadata={
                        "type": "persona",
                        "user_id": user_id,
                        "metadata": attrs,
                ...
                logger.warning(
                    f"Persona generation failed for user {user_id} "
                    f"(attempt {attempt + 1}/{max_retries}): {e}"
                )

        logger.warning(f"Persona generation failed after all retries for user {user_id}")
        return None

    def _update_persona(self, user_id: str) -> None:
        """
        Update user persona from semantic memories.

        Called after add() stores new semantic memories.
        Uses hash comparison to skip writes when content hasn't changed.
        """
        try:
            # Search semantic memories for this user
            search_results = self.vector_store.search(
            ...
            if not search_results:
                logger.debug(f"No semantic memories to update persona for user {user_id}")
                return
            ...
            existing_memory_id = self._get_persona_memory_id(user_id)

            if existing_memory_id:
                existing = self.vector_store.get(vector_id=existing_memory_id)
                if existing:
                    if existing.payload.get("hash") == new_hash:
                        logger.debug(f"Persona hash unchanged for user {user_id}, skipping update")
                        return

                    existing_version = existing.payload.get("version", 1)
                    embeddings = self.embedding_model.embed(persona_text, "update")
                    self._update_memory(
            ...
            logger.warning(f"Persona update failed for user {user_id}: {e}")
            # Non-fatal: semantic memory is already stored

    def _delete_persona_if_exists(self, user_id: str) -> None:
        """
        Delete persona record if it exists.

        Called when semantic memories are deleted (cascade behavior).
        """
        memory_id = self._get_persona_memory_id(user_id)
        if memory_id:
            try:
...
class AsyncMemory(MemoryBase):
    ...
    async def add(
        ...
        *,
        ...
        if effective_filters.get("user_id"):
            try:
                await self._update_persona(effective_filters["user_id"])
            except Exception as e:
                logger.warning(f"Persona update failed for user {effective_filters['user_id']}: {e}")
    ...
    async def delete(self, memory_id):
        ...
        if user_id:
            try:
                await self._delete_persona_if_exists(user_id)
            except Exception as e:
                logger.warning(f"Failed to delete persona for user {user_id}: {e}")
    ...
    # =========================================================================
    # Persona (User Profile) Methods — Async Versions
    # =========================================================================

    async def get_persona(self, user_id: str) -> Optional[str]:
        """Async version of get_persona()."""
        memory_id = self._get_persona_memory_id(user_id)
        if memory_id:
            memory = await asyncio.to_thread(self.vector_store.get, vector_id=memory_id)
            if memory and memory.payload.get("data"):
                return memory.payload["data"]
        return await self._generate_persona_from_semantic_memories(user_id)

    async def get_persona_metadata(self, user_id: str) -> Optional[Dict[str, Any]]:
        """Async version of get_persona_metadata()."""
        memory_id = self._get_persona_memory_id(user_id)
        if not memory_id:
            return None
        memory = await asyncio.to_thread(self.vector_store.get, vector_id=memory_id)
    ...
    def _get_persona_memory_id(self, user_id: str) -> Optional[str]:
        """Query the history table for the persona record memory_id (sync, no await needed)."""
        return self.db.get_memory_id(user_id, memory_type="persona")

    async def _extract_persona_attributes(
        self, facts: List[str], max_retries: int = 3
    ) -> Optional[Dict[str, Any]]:
        """Async version of _extract_persona_attributes()."""
        from mem0.configs.prompts import PERSONA_EXTRACTION_PROMPT
    ...
    async def _generate_persona_text(self, attributes: Dict[str, Any]) -> str:
        """Async version of _generate_persona_text()."""
        from mem0.configs.prompts import PERSONA_TEXT_GENERATION_PROMPT
    ...
    async def _generate_persona_from_semantic_memories(self, user_id: str) -> Optional[str]:
        """Async version of _generate_persona_from_semantic_memories()."""
        max_retries = 3
        for attempt in range(max_retries):
            try:
                ...
                attrs = await self._extract_persona_attributes(facts)
                if not attrs:
                    logger.warning(f"Failed to extract persona attributes for user {user_id}")
                    continue

                persona_text = await self._generate_persona_text(attrs)
                persona_hash = hashlib.md5(
                    json.dumps(attrs, sort_keys=True, ensure_ascii=False).encode()
                ).hexdigest()

                persona_embeddings = await asyncio.to_thread(
                    self.embedding_model.embed, persona_text, "add"
                )
                await self._create_memory(
                    data=persona_text,
                    existing_embeddings={persona_text: persona_embeddings},
                    metadata={
                        "type": "persona",
                        "user_id": user_id,
                        "metadata": attrs,
    ...
    async def _update_persona(self, user_id: str) -> None:
        """Async version of _update_persona()."""
        try:
            embeddings = await asyncio.to_thread(
                self.embedding_model.embed,
                "职业 性格 偏好 沟通风格 技能",
            ...
            if existing_memory_id:
                existing = await asyncio.to_thread(
                    self.vector_store.get, vector_id=existing_memory_id
                )
                if existing:
                    if existing.payload.get("hash") == new_hash:
                        logger.debug(f"Persona hash unchanged for user {user_id}, skipping update")
                    ...
                    existing_version = existing.payload.get("version", 1)
                    persona_embeddings = await asyncio.to_thread(
                        self.embedding_model.embed, persona_text, "update"
                    )
    ...
    async def _delete_persona_if_exists(self, user_id: str) -> None:
        """Async version of _delete_persona_if_exists()."""
        memory_id = self._get_persona_memory_id(user_id)
        if memory_id:
            try:
                await asyncio.to_thread(
